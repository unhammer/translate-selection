...
(defcustom translate-selection-max-length 30
  "Don't translate selections longer than this."
  :type 'integer
...
(defun translate-selection-mouse-drag (start-event)
  "Translate region after mouse drag (if it seems short but not empty).
START-EVENT passed to `mouse-drag-region'."
  (interactive "e")
  (tooltip-hide)
  (funcall 'mouse-drag-region start-event)
  (let* ((selection (buffer-substring-no-properties (region-beginning) (region-end)))
    ...
    (when (and (> len 0)
               (<= len translate-selection-max-length))
...
the mouse will send it through the \"trans\" command."
 :init-value nil
 :lighter "📚"
 :keymap '(([down-mouse-1] . translate-selection-mouse-drag)
